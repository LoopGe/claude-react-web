...
export function SnippetsManagerDialog({ open = true, api, onClose }: Props) {
  ...
  // Override the focus trap's default first-focusable-child behaviour. The
  // trap's effect also runs after mount; this one runs after-after so it
  // wins. We also intentionally don't re-focus on every state change — the
  // user may have moved focus elsewhere and we shouldn't yank it back.
  useEffect(() => {
  ...
  return (
    <Overlay
      variant="perm"
      ariaLabel="Manage composer snippets"
      cardClassName="snippets-manager-card"
      open={open}
      onClose={tryClose}
    >
