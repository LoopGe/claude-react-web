...
function seedCache(id: string): void {
  localStorage.setItem(
    STORAGE_PREFIX + id,
    JSON.stringify({ v: 1, savedAt: Date.now(), messages: [], items: [] }),
  )
}
...
describe('sessionStoreRegistry.delete', () => {
  ...
  it('clears the transcript cache for a session with a live in-memory store', () => {
  ...
  it('clears an orphan cache even when no in-memory store exists', () => {
