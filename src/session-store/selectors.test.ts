...
describe('useSessionTaskCounts', () => {
  ...
  it('counts all non-terminal tasks as `all` and excludes skipTranscript from `waiting`', () => {
    ...
    // a + c are non-terminal (2); only a counts toward Waiting (c is
    // ambient/skipTranscript).
    expect(result.current).toEqual({ all: 2, waiting: 1 })
  })
