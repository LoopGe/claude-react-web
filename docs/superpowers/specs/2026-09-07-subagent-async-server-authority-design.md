...
- 前置裁决(用户已批准):更大重构;纯 server 权威(Q2);方案 B(ack 嗅探仅作防误结算守卫)。D1=删(seed 不种 `isAsync`);D2=B(ack 命中不翻 `background`,配套 sweep 保险 §3.4)。
...
- Client:ack 守卫按 D2=B 定稿——命中 launch-ack **只跳过合并**,不翻状态、不设 `isAsync`(§3.3)。
- Client:turn-end sweep 保险(§3.4,D2=B 配套)。
- Client:child-after-result 分支删除 `isAsync` 翻转,保留 `endedAt` 推进与子帧文本/工具计数捕获(§3.5)。
- Client:task_notification 分支删除 `isAsync:true`(§3.6)。
- Client:seed 不再种 `isAsync`(D1=删,§3.7)。
- 上述全部回归测试(§5)、注释/CLAUDE.md 对账(§3.8)。
...
- `raw` 类型已含 `is_backgrounded?: unknown`(探针日志读取同字段,实现时一并收编/移除日志)。
- 效果:sync 注册瞬间即带 `false`,async 带 `true`;每次 fold 后 `pushTasksSnapshot` 推的 snapshot 立刻携带权威值;terminal 记录经 spread 保留该值。
- frame-less 后台 dispatch 不需改(watcher seed + 立即 push 已存在)。前台 frame-less dispatch(若有 CLI 不发 `task_started`)→ 无任务记录,`isAsync` 保持 undefined(§4 记录,接受)。
...
要点:
- **`false` 也写**:sync 卡即使 input 没带 `run_in_background` 也能标 sync;纠正任何旧值(D1 后无 seed,初始 undefined 也由此落定)。
- **terminal 也写**:刷新后靠 server 保留的 terminal 记录恢复徽标(§4 cap 限制)。
- **rescue 从「ack 误结算」收敛为「server 说它是 live 后台任务但记录被 detach 文本误结算」**;launch-ack 不再由文本翻 `background`(见 §3.3)。

### 3.3 Client:ack 守卫(D2=B:只跳过合并,不翻状态、不设 `isAsync`)

`result-merge` 分支(`updateIndexesMirror`)现状命中 launch-ack 时翻 `background`。**D2=B** 改为:命中 launch-ack **只跳过合并**——不把 ack 当真实结果结算,不设 `endedAt/result`,**不翻 `background`**,**不设 `isAsync`**。记录保持 `running`,由 §3.2 的 snapshot(`isBackgrounded:true`)负责翻 `background`。

```ts
// isAck = !isError && /^async agent launched successfully/i.test(ackText)   // 纯正则(D1 后无 seed 免疫)
if (isAck) continue   // 或等效:命中则不改写记录;记录停留在 'running'
```

- `isAck` 判定的唯一职责 = **防误结算**:launch-ack 文本不是 subagent 的真实输出,绝不能进 `result`。
- Ctrl+B detach ack(文本形如 "Subagent moved to background",**非** launch 签名)不命中 → 仍会被当作真实结果合并成 `done`+假 result → 由 §3.2 `rescueSettled`(live 非 terminal + `isBackgrounded:true`)救回,或由 §3.6 通知完成分支用真实 summary 覆盖。
- D2=B 的直接后果:snapshot 到达前,async 记录停在 `running`(无徽标、chip 显示 running)。§3.4 的 sweep 保险兜住「此时 parent turn 结束」的交错。

### 3.4 Client:turn-end sweep 保险(D2=B 配套)

`sweepAtTurnEnd` 现状:`running → interrupted`、`background → pending`。D2=B 下 async 记录在 snapshot 前是 `running`,需防误杀:

对 `running` 记录,若 `mirror.tasks` 中存在该 `toolUseId` 的**非 terminal** 任务记录且 `isBackgrounded === true` → 置 `pending`(仍在后台跑),而不是 `interrupted`。其余 `running`(无任务记录 / 任务非后台)维持 `interrupted`。

触发场景(snapshot 先于 seed 到达时,`mirror.tasks` 有任务但 activeSubagents 尚无该 toolUseId 记录,随后 seed 建 `running`;或重放与 live snapshot 交错):server 已认定它是 live 后台任务,sweep 不应反悔。

实现:sweep 前对 `mirror.tasks` 建一次 toolUseId→task 临时 Map(仅当存在 `running` 记录时构建;turn-end 一次,可忽略)。

### 3.5 Client:child-after-result 只保留计时/捕获,删 `isAsync` 翻转

现状(async-detector)对 `background/pending` 记录做两件事:(i) 推进 `endedAt`(计时)+ 子帧文本/工具计数捕获进 `result`;(ii) 顺带 `asyncChanged ? { isAsync: true }`。

改为**只保留 (i),删除 (ii)**。`background` 状态现在只由 §3.2(snapshot)置位(D2=B 下 ack 不翻),该分支语义自洽:
...
- 残余:ack 已跳过、snapshot 未到时子帧先到(记录仍 `running`)→ 不推进计时,待 snapshot 翻 `background` 后才开始;ms 级计时起点的近似,接受。

### 3.6 Client:task_notification 只做完成信号
...
修复效果(实测 bug):纯前台 sync 的完成通知到达时,记录 `status='done'`、`isAsync` 为 §3.2 snapshot 给的 `false` → **不再误标 async**。通知对 sync 卡是幂等完成(sync 真实输出已由 tool_result 合并,通知 summary 覆盖为同内容,无害)。

### 3.7 Client:seed 不再种 `isAsync`(D1=删)

现状 `src/session-store/normalize.ts` `getSubagentStarts` 读 `input.run_in_background` 种 `isAsync`。**D1=删**:改为**不种 `isAsync`**(记录初始 `isAsync: undefined`),async/sync 完全交给 §3.2。

- ack 守卫随之简化为纯正则:删 `existing.isAsync !== false` 与 `existing.isAsync === true` 两子句,`isAck = !isError && /^async agent launched successfully/i.test(ackText)`。理由:无 seed 后 `isAsync===false` 的「显式 sync 免疫」保护不再可得,但其唯一价值是防「sync 真实输出恰好以 ack 短语开头」——该病态情形即使误判,§3.6 通知(接受 `running`)会以真实 summary 覆盖,自愈。
...
- `normalize.ts` `getSubagentStarts` 注释同步(D1)。
...
3. **新回归(核心 bug)**:纯前台 sync——Agent tool_use(`run_in_background:false`)→ snapshot(`isBackgrounded:false`,模拟 task_started 折叠)→ 真实输出 tool_result → `task_notification(completed)` → 断言 `status==='done'` 且 **`isAsync===false`**(喂实测帧序列;D1 下 `false` 来自 snapshot)。
4. **新**:sync 无 flag 卡,收到 `task_notification` 前先来一张 terminal snapshot(`isBackgrounded:false`)→ `isAsync===false`。
5. `TASKS_SNAPSHOT` 权威覆盖:记录初始 `isAsync` 为 undefined 时来一张 `isBackgrounded:true` 的 snapshot → `isAsync===true` 且 `running→background`;`false` → `isAsync===false` 且不翻状态。
...
9. ack 守卫(D2=B):launch-ack tool_result → **不结算、不设 result/endedAt、状态保持 `running`、`isAsync` 不被文本置位**。
10. sweep 保险(§3.4):`running` 记录 + `mirror.tasks` 有非 terminal `isBackgrounded:true` 任务 → parent turn 结束时置 `pending`;无任务记录 → `interrupted` 不回退。
11. 改既有:凡断言 ack / child-after-result / task_notification 设 `isAsync` 的用例,改为「先喂一张带 `isBackgrounded` 的 snapshot 再断言」(如 `flips a background subagent to done via…task_notification`、replay-ordering 测试靠其 terminal snapshot 继续通过)。
12. **删既有(D1)**:凡断言「seed 把 `run_in_background` 种成 `isAsync`」的用例(reducer.test.ts 约 :1300-1345 段的 `Seeded false from the explicit run_in_background: false flag` 等)删除或改写为 snapshot 驱动。

**探针清理**:移除 §3.8 所列临时日志后,`debug.ts` 恢复原状。

## 6. 风险与回滚

- **帧顺序 unspecified**:snapshot / ack / notification 相对顺序不保证。设计不依赖具体顺序:任何顺序下 `isAsync` 都收敛到 server 值(§3.2 双向写 + §3.6 通知只完成)。残余瞬态:async 卡在 ack 与首张 snapshot 间为 `running`/无徽标(ms 级)。§3.4 sweep 保险兜住 snapshot 先到而 seed 后到的交错。若实测发现可感知闪烁,再做补偿(不属本 spec 范围)。
...
- **D1 seed**:删——`getSubagentStarts` 不再读 `run_in_background` 种 `isAsync`,初始 undefined,ack 守卫纯正则。(批准)
- **D2 ack 命中**:B——只跳过合并,不翻 `background`、不设 `isAsync`;配套 §3.4 sweep 保险。(批准)
