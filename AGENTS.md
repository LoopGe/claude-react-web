...
- `src/session-store/normalize.ts` — `getSubagentStarts()` scans for Agent / Task / Explore tool_use calls with no matching tool_result. Feeds `WorkingBubble`'s in-flight subagent display: up to `SUBAGENT_INLINE_LIMIT` (2) render as individual chips, above that they collapse into one `SubagentSwarmPill` (`src/components/message-list/SubagentSwarm.tsx`) whose popover carries the per-agent detail (`progressSummary` / `lastToolName` / elapsed) and drills into `SubagentOverlay`. The aggregate/inline choice is made on the fan-out's PEAK width and only resets when it fully drains, so the bar's height can't oscillate mid-turn and the open popover isn't yanked away. `.working-bar` therefore carries no `max-height` — the old 120px clip silently ate the tail of a wide fan-out, starting with the "+N more" badge that rendered last.
