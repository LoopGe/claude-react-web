...
export function buildGitWriteRouter(sm: SessionManager): Hono {
  ...
  app.post('/sessions/:id/git/commit-message', async (c) => {
    ...
    // 500 — the caller always gets a usable message. The target carries the
    // session's resolved model AND its own profile's endpoint/token, so a
    // session pinned to a non-active profile doesn't spend the other one.
    const result = await generateCommitMessage(text, { target: sm.auxTargetFor(id, 'commitMessage') })
    if (result.fallback) {
